...
"use client";
import { TSetNewPasswordSchema, setNewPasswordSchema } from "@/utils/zod/zod";
import React from "react";

import {SubmitHandler, useForm} from "react-hook-form";

import {zodResolver} from "@hookform/resolvers/zod";
import { handleOnSetNewPassword } from "@/ServerActions/User";
...
const SetNewPassword = () => {
    ...
    if(!token.token) return new Error("Invalid Request");

    const submit: SubmitHandler<TSetNewPasswordSchema> = async (data) => {
        dispatch(setIsLoading(true));
        const res = await handleOnSetNewPassword(data,`${token.token}`);
        console.log(res);
        if (res.errors) {
            Object.keys(res.errors).map((key: string) => {
                setError(key as "password" | "confirmPassword", {
  ...
  return (
    <main className="flex overflow-y-hidden gap-5 px-3 py-5 justify-center rounded-md   h-full z-50  flex-col mx-auto max-w-2xl">
      <div className=" border px-5 py-10 rounded-md  gap-5 flex flex-col border-slate-800">
        <div className=" ">
          <h1 className="text-white text-xl">Set New Password </h1>
        </div>
