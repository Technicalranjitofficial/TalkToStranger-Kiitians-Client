...
import { SendResetPasswords } from "@/ServerActions/User";
...
const Reset = () => {
  ...
  const errMessage = useAppSelector((state)=>state.AuthSlice.errMessage);
  ...
  return (
    <main className="flex overflow-y-hidden gap-5 px-3 py-5 justify-center rounded-md   h-full z-50  flex-col mx-auto max-w-2xl">
      <div className=" border px-5 py-10 rounded-md  gap-5 flex flex-col border-slate-800">
        <div className=" ">
          <h1 className="text-white text-xl">Reset Your Password </h1>
        </div>
        ...
        <form  className="text-red-500">
          ...
          { errMessage &&  errMessage.length>0 &&<span className="mb-3">{errMessage}</span>}
