...
import { login, signup } from "@/ServerActions/User";
...
const SignIn = () => {
  ...
  const submit: SubmitHandler<TLoginSchema> = async (data) => {
    console.log(data);
    ...
    if (zodErrors) {
      Object.keys(zodErrors).map((key: string) => {
        setError(key as "email" | "password", {
          message: zodErrors[key as keyof typeof zodErrors],
          type: "server",
        });
      });
    }
  ...
  return (
    <main className="flex overflow-y-hidden gap-5 px-3 py-5 justify-center rounded-md   h-full z-50  flex-col mx-auto max-w-2xl">
      <div className=" border px-5 py-10 rounded-md  gap-5 flex flex-col border-slate-800">
        <div className=" ">
          <h1 className="text-white text-xl">Login and Start Chatting </h1>
        </div>
